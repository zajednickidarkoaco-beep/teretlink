...
import { Package, Truck as TruckIcon, Activity, Lock, TrendingUp, Plus, ArrowUpRight, Zap, MapPin, Calendar, Phone } from 'lucide-react';
          ...
          {matchedLoads.length > 0 && (
            <div className="space-y-2">
              ...
              {matchedLoads.map(load => (
                <Card key={load.id} className="p-4 border-brand-400/20">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge variant="info">TURA</Badge>
                        {load.price && (
                          <span className="font-bold text-text-main font-mono text-sm ml-auto">{load.price} {load.currency}</span>
                        )}
                      </div>
                      <p className="font-bold text-text-main text-sm flex items-center gap-2">
                      ...
                      {canViewContact(load.userId) && load.contactPhone ? (
                        ...
                        <Link to="/loads">
                          <Button variant="outline" size="sm" className="text-xs gap-1">
                            Pogledaj <ArrowUpRight className="h-3 w-3" />
                          </Button>
                        </Link>
                      )}
                    </div>
          ...
          {matchedTrucks.length > 0 && (
            <div className="space-y-2">
              ...
              {matchedTrucks.map(truck => (
                <Card key={truck.id} className="p-4 border-brand-400/20">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="flex items-center gap-1 text-[10px] font-bold text-brand-400 bg-brand-400/10 px-2 py-0.5 rounded-full border border-brand-400/20">
                        ...
                        {truck.weightCapacity && (
                          <span className="text-xs text-text-muted font-medium ml-auto">{truck.weightCapacity}t</span>
                        )}
                      </div>
                      <p className="font-bold text-text-main text-sm flex items-center gap-2">
                        <MapPin className="h-3.5 w-3.5 text-text-muted flex-shrink-0" />
